...
public class DesertAndCakeFragment extends Fragment {
    private FragmentDesertAndCakeBinding binding;
    private DesertsAdapter adapter;
    private FirebaseFirestore firestore;
    private TextView textViewPrice;
    private ArrayList<ModelDeserts> desertList;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding = FragmentDesertAndCakeBinding.inflate(inflater, container, false);
        View view = binding.getRoot();

        firestore = FirebaseFirestore.getInstance();
        desertList = new ArrayList<>();
        adapter = new DesertsAdapter(getActivity(),desertList);
...
//        LoadingDataFunc();
    ...
    private void LoadingDataFunc() {
        desertList.clear();
        firestore.collection("deserts").orderBy("productId", Query.Direction.ASCENDING).addSnapshotListener((value, error) -> {
            if(error != null)
                Toast.makeText(getActivity(), "document loaded", Toast.LENGTH_SHORT).show();
            for(DocumentChange dc : Objects.requireNonNull(value).getDocumentChanges()){
                if(dc.getType()== DocumentChange.Type.ADDED){
                    desertList.add(dc.getDocument().toObject(ModelDeserts.class));
                }
                adapter.notifyDataSetChanged();
            }
        });
    }
    public void setupTextViewListeners(ModelCoffees coffee ) {
    ...
    private void loadInitialData() {
        Query initialQuery = firestore.collection("deserts")
            ...
            if (task.isSuccessful()) {
                ...
                if (snapshot != null && !snapshot.isEmpty()) {
                    for (DocumentSnapshot document : snapshot.getDocuments()) {
                        desertList.add(document.toObject(ModelDeserts.class));
                    }
                    if (snapshot.size() > 0) {
    ...
    private void loadMoreData() {
        ...
        Query nextQuery = firestore.collection("deserts")
                .orderBy("productId", Query.Direction.ASCENDING)
